...
<div class="x-nav">
      <span class="layui-breadcrumb">
        <a href="">首页</a>
        <a href="<%=request.getContextPath()%>/findStudent">学生信息</a>
      </span>
    <a class="layui-btn layui-btn-small" style="line-height:1.6em;margin-top:3px;float:right" href="<%=request.getContextPath()%>/findStudent" title="刷新">
        <i class="layui-icon" style="line-height:30px">ဂ</i></a>
</div>
<div class="x-body">
    <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so" action="<%=request.getContextPath()%>/findStudent" >
...
<c:forEach items="${pi.list}" var="student">
        <tr>
            ...
            <td>
                <a title="编辑"    id= "updateEdit"    href="<%=request.getContextPath()%>/findStudentById?s_id=${student.s_id}">
...
<script>
    ...
    layui.use(['jquery', 'excel','form','layer','laydate'], function(){
        ...
        form.on('submit(toolbarDemo)', function(){
            ...
            $.ajax({
                url: '<%=request.getContextPath()%>/exportstudentlist',
                type: 'post',
                dataType: 'json',
    ...
    function member_del(obj,s_id){
        layer.confirm('确认要删除吗？',function(index){
            //发异步删除数据
           $.get("<%=request.getContextPath()%>/deleteStudent",{"s_id":s_id},function (data) {
                if(data =true){
